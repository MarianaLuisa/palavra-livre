...
-- ---------------------------------------------------------------------
-- Inicio de uma rodada diária com blindagem de enum participant_round_status
-- ---------------------------------------------------------------------
...
begin
  ...
  where previous_round.championship_id = championship.id
    ...
    and (
      previous_participation.status is null
      or previous_participation.status::text in ('NOT_STARTED', 'IN_PROGRESS')
      or previous_participation.status::text not in ('COMPLETED', 'FAILED', 'EXPIRED')
    );
  ...
  values (participant.id, round_record.id, 'IN_PROGRESS'::participant_round_status, now())
  on conflict (championship_participant_id, championship_round_id) do update
    set status = case
          when participant_rounds.status::text = 'NOT_STARTED' then 'IN_PROGRESS'::participant_round_status
  ...
  set started_at = coalesce(started_at, now()),
      status = case when status = 'REGISTERED' then 'IN_PROGRESS'::participation_status else status end
...
-- ---------------------------------------------------------------------
-- Avaliacao de palpites na rodada do campeonato
-- ---------------------------------------------------------------------
create or replace function public.cd_evaluate_guess(
  guess text,
  answer text,
  answer_normalized text default null
)
returns jsonb
language plpgsql
immutable
...
begin
  if char_length(normalized_guess) <> word_length then
    word_length := 5;
    normalized_guess := rpad(normalized_guess, 5, 'a');
    normalized_answer := rpad(normalized_answer, 5, 'a');
  end if;
  ...
  -- Primeira passagem: posicoes corretas (verde)
  ...
  -- Segunda passagem: presentes fora de posicao (amarelo)
  for position_index in 1 .. word_length loop
...
-- ---------------------------------------------------------------------
-- Submissão de tentativa de palavra na rodada
-- ---------------------------------------------------------------------
...
begin
  ...
  if not found then
    ...
    values (participant.id, round_record.id, 'IN_PROGRESS'::participant_round_status, now())
    returning * into participant_round;
  end if;

  if participant_round.status::text in ('COMPLETED', 'FAILED', 'EXPIRED') then
  ...
  next_attempt_number := coalesce(participant_round.attempts_used, 0) + 1;
  ...
  set attempts_used = next_attempt_number,
      ...
      status = case
        when not round_finished then 'IN_PROGRESS'::participant_round_status
        when solved_total >= round_record.board_count then 'COMPLETED'::participant_round_status
        else 'FAILED'::participant_round_status
      end,
